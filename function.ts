...
function checkNameLength(name: string): number {
  return name.length;
}
...
function compareToThree(num: number): string {
  if (num > 3) {
...
function hasAgeGreaterThanThree(users: User[]): boolean {
...
function searchName(users: User[], userInput: string): string {
...
function searchNameAlternative(users: User[], userInput: string): string {
